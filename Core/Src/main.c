...
volatile int   state        = Savings_Mode;  // current system state
volatile int  g_log_enabled    = 0;          // SD card logging enabled flag
volatile int  g_temperature      = 0;          // temperature in degree C integer part
/* USER CODE END PM */
...
/**
  * @brief  Default Task: display and periodic temperature read
  */
void StartDefaultTask(void *argument)
{
    int last_state = -1;
...
/**
  * @brief  Button Task: read PA0/PA1 and update state
  */
void StartButtonTask(void *argument)
{
    for (;;)
    {
        ...
        if (!(idr & (1U << 0)))
        {
            ...
            {
                state = Working_Mode;
                g_log_enabled = 0;   // Khi vừa bật, chưa ghi
            }
            else     // đang WORKING → tắt máy
            {
                state = Savings_Mode;
                g_log_enabled = 0;   // tắt ghi
            }
        ...
        else if (!(idr & (1U << 1)))
        {
            if (state == Working_Mode)
            {
                g_log_enabled = !g_log_enabled;  // toggle
            }
...
/**
  * @brief  SDCard Task: mount and append temperature log in a loop
  */
void StartSdcardTask(void *argument)
{
    char line[64];
    ...
    for (;;)
    {
        if (state == Working_Mode && g_log_enabled)
        {
            ...
            snprintf(line, sizeof(line), "%lu: %s°C\r\n",
                     (unsigned long)t_sec,
                     tempStr);
